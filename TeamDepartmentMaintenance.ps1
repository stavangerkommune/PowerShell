...
$smtpFrom = "<Teams.Maintenance@contoso.com>"
...
$serverName = "sql-SVGKOMM"
$databaseName = "SVGKOMM"
$query = "SELECT group_name FROM groups WHERE (group_office365 = 1)"
...
[System.Collections.ArrayList]$allDepsFromSQL = $dt | Select-Object -ExpandProperty group_name
...
### CREDENTIALS ###
$Username = "O365ServiceAccount@contoso.com"
...
foreach ($depFromSQL in $allDepsFromSQLClone)
{
    ...
    $depFromAD = Get-ADGroup -Filter {Name -eq $depFromSQL } -SearchBase 'OU=Departments,OU=Organization,DC=contoso,DC=com' -Properties DistinguishedName,SamAccountName,extensionAttribute3
    ...
    {
        if ($depTeamID -eq "0") # extensionAttribute3 can be set to 0 in AD to "un-link" AD dep from the Team
        {
...
foreach ($depFromSQL in $allDepsFromSQL)
{
    ...
    $depFromAD = Get-ADGroup -Filter {Name -eq $depFromSQL } -SearchBase 'OU=Departments,OU=Organization,DC=contoso,DC=com' -Properties DistinguishedName,SamAccountName,ManagedBy,extensionAttribute3
    if($Error)
    {
        $ex = $Error[0]
